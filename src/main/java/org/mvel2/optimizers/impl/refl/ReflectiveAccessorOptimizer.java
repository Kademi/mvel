        ...
        try {
            if (!MVEL.COMPILER_OPT_ALLOW_OVERRIDE_ALL_PROPHANDLING) {
                while (cursor < end) {
                    ...
                    if (curr != null) {
                        if (this.pCtx.getFiltersManager() != null) {

                            if (!this.pCtx.getFiltersManager().exposeObjectToScripts(curr)) {
                                throw new RuntimeException("Cannot access object: " + curr.getClass().getName());
                            }
                        }
                    }
